...
export function AdminOrderDetailModal({
  ...
  return (
    <Modal
      opened={opened}
      onClose={onClose}
      title={order ? `Order ${order.shortRef}` : 'Order Details'}
      size="xl"
      centered={!isMobile}
      fullScreen={isMobile}
                ...
                <Stack gap="sm">
                  <Group justify="space-between" align="flex-start">
                    <div>
                      ...
                      <Text size="sm" style={{ color: 'var(--tt-cream-muted)' }}>{order.shortRef}</Text>
                    </div>
                ...
                <Stack gap="md">
                  ...
                  <div className="admin-mobile-detail-list">
                    {order.items.map((item) => (
                      <div key={item.id} className="admin-mobile-detail-row">
                        ...
                        <Text fw={900}>{formatMoney(item.lineTotalCents, order.currency)}</Text>
                      </div>
                    ))}
                  </div>
                  <Table.ScrollContainer minWidth={680} className="admin-desktop-detail-table">
              ...
              <Paper withBorder radius="md" p="md" className="enterprise-panel">
                <Stack gap="sm">
                  ...
                  {order.history.map((item) => (
                    <div key={item.id}>
                      <Text size="sm" fw={700}>
                        {item.label || adminOrderStatusLabels[item.status] || item.status}
                      </Text>
