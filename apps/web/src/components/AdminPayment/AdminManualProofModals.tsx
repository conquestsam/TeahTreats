      ...
      {proof ? (
        <ScrollArea.Autosize mah="calc(100vh - 180px)" offsetScrollbars>
          <Stack gap="lg" pr="sm">
            <SimpleGrid cols={{ base: 1, md: 2 }} spacing="lg">
              <Paper withBorder radius="md" p="md" className="enterprise-panel">
                <Stack gap="sm">
                  ...
                  <Text fw={900}>Review Payment</Text>
                  <Text size="sm" style={{ color: 'var(--tt-cream-muted)' }}>
                    Order {proof.shortOrderRef} • Receipt {proof.receiptReference}
                  </Text>
                  {proof.receiptUrl ? (
                    ...
                    <div className="admin-proof-receipt-frame admin-proof-receipt-empty">
                      <Text fw={850}>No receipt attached</Text>
                      <Text size="sm" style={{ color: 'var(--tt-cream-muted)' }}>
                        Review the payment amount, customer note, and order details before taking action.
                      </Text>
                    </div>
                  )}
                  <Group gap="xs">
              ...
              <Paper withBorder radius="md" p="md" className="enterprise-panel">
                <Stack gap="sm">
                  ...
                  <Text size="sm">Payment instructions: {proof.methodInstructions || 'Not provided'}</Text>
                  <Text size="sm">Customer note: {proof.note ?? 'No note'}</Text>
                </Stack>
              </Paper>
            </SimpleGrid>
              ...
              <Stack gap="md">
                <Group justify="space-between" align="flex-start">
                  <div>
                    ...
                    <Text fw={800}>{proof.shortOrderRef}</Text>
                ...
                <div className="admin-mobile-detail-list">
                  {proof.items.map((item) => (
                    <div key={`${item.productName}-${item.skuName}`} className="admin-mobile-detail-row">
                      <ItemThumb imageUrl={item.imageUrl} />
                      <div>
                        <Text fw={850}>{item.productName}</Text>
                        <Text size="sm" style={{ color: 'var(--tt-cream-muted)' }}>{item.quantity}x {item.skuName}</Text>
                      </div>
                      <Text fw={900}>{formatMoney(item.lineTotalCents, proof.currency)}</Text>
                    </div>
                  ))}
                </div>
                <Table.ScrollContainer minWidth={620} className="admin-desktop-detail-table">
            ...
            <Paper withBorder radius="md" p="md" className="enterprise-panel admin-mobile-checklist">
              <Stack gap="sm">
                <Text size="xs" tt="uppercase" fw={900} style={{ color: '#ffd98a', letterSpacing: '0.08em' }}>
                  Review checklist
                </Text>
                <ChecklistRow label={`Receipt amount matches ${formatMoney(proof.orderTotalCents, proof.currency)}`} />
                <ChecklistRow label={`Payment method is ${proof.methodLabel}`} />
                <ChecklistRow label="Customer and order details match" />
              </Stack>
            </Paper>
          </Stack>
        </ScrollArea.Autosize>
...
function ChecklistRow({ label }: { label: string }) {
  return (
    <div className="admin-mobile-checklist-row">
      <span>✓</span>
