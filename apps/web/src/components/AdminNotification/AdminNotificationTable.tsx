          ...
          <Table.Tbody>
            {notifications.map((notification) => (
              <Table.Tr key={notification.id}>
                ...
                <Table.Td>
                  <Text size="sm" c={notification.lastError ? 'red' : 'dimmed'} lineClamp={2}>
                    {notification.lastError ?? (notification.sentAt ? `Sent ${new Date(notification.sentAt).toLocaleString()}` : 'Waiting')}
                  </Text>
                </Table.Td>
