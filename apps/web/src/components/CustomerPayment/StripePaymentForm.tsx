...
function StripeCheckoutForm({ onSuccess, returnUrl }: { onSuccess: () => void; returnUrl: string }) {
...
export function StripePaymentWrapper({
    ...
    return (
      <div className="tt-state-card" style={{ padding: 24 }}>
        ...
        <p style={{ color: 'var(--tt-cream-muted)', fontSize: '0.85rem', margin: 0 }}>
          Card payment is not ready here yet. Please use another payment option or contact support.
        </p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <StripeCheckoutForm onSuccess={onSuccess} returnUrl={returnUrl} />
    </Elements>
  );
}
