...
export function AdminAuthGate({ children }: AdminAuthGateProps) {
  ...
  const currentUserQuery = useAdminCurrentUserQuery(pathname !== authRedirects.adminLogin);
  const logoutMutation = useAdminLogoutMutation();
  useAdminRealtime(pathname !== authRedirects.adminLogin && currentUserQuery.isSuccess);

  useEffect(() => {
    if (currentUserQuery.isError) {
      redirectOnce(authRedirects.adminLogin);
    }
  }, [currentUserQuery.isError]);

  if (pathname === authRedirects.adminLogin) {
    return <>{children}</>;
  }
