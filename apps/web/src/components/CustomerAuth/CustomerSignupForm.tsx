...
export function CustomerSignupForm({
  ...
  const redirectTo = claimOrderId ? `/orders/${claimOrderId}` : '/account';
  const googleUrl = buildCustomerOAuthStartUrl('google', redirectTo);
  ...
  return (
    <form onSubmit={form.onSubmit(onSubmit)}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {form.errors.name || form.errors.email || form.errors.phone || form.errors.password ? (
        ...
        <div style={{ textAlign: 'center', marginTop: 10, fontSize: '0.85rem', color: 'var(--tt-cream-muted)' }}>
          ...
          <Link href={claimOrderId ? `/login?claimOrderId=${claimOrderId}` : '/login'} style={{ color: 'var(--tt-gold-light)', fontWeight: 600, textDecoration: 'none' }}>
            Sign in
          </Link>
        </div>
      </div>
